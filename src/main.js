...
const NAV_GROUPS = {
  'Command': ['Dashboard','Fleet Overview','Vessels','Vessel Profile'],
  'Voyage Ops': ['Voyages','Navigation','Cargo','Port Calls'],
  'Crew': ['Crew List','Crew Certificates','Watch Schedule'],
  'Maintenance': ['PMS','Work Orders','Machinery','Inventory','Critical Quotes'],
  'Safety & Compliance': ['PTW Register','Inspections','Incidents','eORB','Certificates'],
  'Workspace': ['Agent Desk','Analytics','Settings']
};
...
const popupData = {
  notifications: { title: 'Notifications', body: '<b>4 urgent operating items require attention.</b><br><br>• PTW #0047 needs Master signature.<br>• eORB #044 requires Chief Engineer countersignature.<br>• PO-291 is waiting for vendor quote approval.<br>• IOPP certificate expires in 21 days.', fields: [['Queue owner','Captain'],['Highest severity','Critical'],['Due window','Today'],['Escalation','Enabled']] },
  profile: { title: 'Operator Profile', body: 'Francis is logged into the demo workspace as the fleet administrator. This panel will hold account settings, role permissions, vessel access, MFA status, and audit preferences.', fields: [['Role','Fleet Admin'],['Workspace','Neptune Demo'],['Access','All vessels'],['MFA','Planned']] },
  quick: { title: 'Quick Create', body: 'Start a new operating record. These buttons simulate creating records that would save into the real Neptune database.', quick: ['New PTW','New Work Order','New Port Call','New Certificate Action','New Incident','New Purchase Request','Upload Evidence','New Crew Note'] },
  export: { title: 'Export Package', body: 'Export will generate a signed PDF/CSV evidence package for the current module, including timestamps, owners, attached documents, and approval history.', fields: [['Format','PDF + CSV'],['Module','Current page'],['Audit trail','Included'],['Delivery','Email + download']] },
  comment: { title: 'Comment Thread', body: 'Comments stay connected to the vessel, module, and record. Mentions notify the Captain, Chief Engineer, Chief Officer, shore manager, or procurement owner.', fields: [['Thread','Operational review'],['Participants','Captain, HOD, Shore'],['Status','Open'],['Visibility','Internal']] },
  refresh: { title: 'Activity Refreshed', body: 'The activity stream has been refreshed. In production this pulls the latest vessel events, document updates, approvals, and department comments.', fields: [['Last sync','Just now'],['New events','4'],['Source','Fleet workspace'],['Status','Healthy']] },
  access: { title: 'Organization Access Request', body: 'Access request captured. In the real product this sends a request to the workspace administrator and creates an onboarding task.', fields: [['Requester','francis@neptune.local'],['Workspace','Neptune Demo'],['Role requested','Admin'],['Status','Pending']] },
  forgot: { title: 'Password Recovery', body: 'Password recovery emails a secure link and records the event in the workspace security log.', fields: [['Email','francis@neptune.local'],['Method','Secure email link'],['Expires','15 minutes'],['Audit','Recorded']] }
};
...
function loginScreen(){
  return `<main class="login-wrap"><section class="login-card"><div class="neptune-logo"><div class="logo-eye">✦</div><b>NEPTUNE</b></div><p class="eyebrow">Secure Operator Login</p><h1>Welcome to Neptune</h1><p class="muted big">Use the demo credentials below to enter the clickable prototype.</p><form id="login-form" class="login-form"><label>Email<input value="francis@neptune.local" autocomplete="email" /></label><label>Password<input value="neptune" type="password" autocomplete="current-password" /></label><div class="login-row"><span><input type="checkbox" checked /> Remember workspace</span><button type="button" class="text-button" data-popup="forgot">Forgot password?</button></div><button class="primary" type="submit">Enter Command Center →</button><button class="secondary" type="button" data-popup="access">Request organization access</button></form></section><button class="fab" data-popup="quick" aria-label="Quick create">+</button></main>`;
}

function navHtml(){
  return Object.entries(NAV_GROUPS).map(([group, items], index) => {
    const open = items.includes(page) || index < 2 ? 'open' : '';
    return `<details class="nav-section" ${open}><summary><span>${group}</span><small>${items.length}</small></summary><div class="nav-mini-grid">${items.map(p=>`<button class="side-link ${p===page?'active':''}" data-page="${p}"><span>${p}</span></button>`).join('')}</div></details>`;
  }).join('');
}

function shell(){
  return `<div class="app-shell ${sidebar?'menu-active':''}"><aside class="sidebar" aria-label="Neptune navigation"><div class="side-head"><div class="neptune-logo compact"><div class="logo-eye">✦</div><b>NEPTUNE</b></div><button class="icon-btn" data-menu="close" aria-label="Close menu">×</button></div><div class="side-label">Command Modules</div><div class="sidebar-summary"><b>${page}</b><span>${pageCopy[page]?.[0] || 'Active module'}</span></div><nav class="side-nav condensed-nav">${navHtml()}</nav></aside><button class="scrim" data-menu="close" aria-label="Close menu"></button><main class="workspace">${topbar()}${searchOpen ? commandSearch() : ''}${page === 'Dashboard' ? dashboard() : pageTemplate(pageCopy[page] || pageCopy.Dashboard)}</main>${drawerHtml()}<button class="fab" data-popup="quick" aria-label="Quick create">+</button><nav class="bottom-dock" aria-label="Mobile navigation"><button data-page="Dashboard" aria-label="Dashboard">⌂</button><button data-page="Vessels" aria-label="Vessels">▱</button><button data-search="true" aria-label="Search">⌕</button><button data-page="Certificates" aria-label="Certificates">▣</button><button data-menu="open" aria-label="Menu">☰</button></nav></div>`;
}

function topbar(){ return `<header class="topbar"><button class="icon-btn" data-menu="open" aria-label="Open menu">☰</button><button class="search-pill" data-search="true">⌘ Search vessels, crew, PTW, inventory, certificates... <kbd>Ctrl<br/>K</kbd></button><button class="bell" data-popup="notifications" aria-label="Notifications">🔔<i></i></button><button class="profile-pill" data-popup="profile">FE</button></header>`; }

function dashboard(){
  return `<section class="hero-card glass premium-glow" data-drawer="Live Command Center|Neptune is combining vessel records, department workflows, approvals, and evidence into one operating layer."><p class="eyebrow">Live Command Center</p><h1>Every vessel, every approval, every risk signal — visible in one place.</h1><p class="hero-copy">Neptune consolidates the bridge, engine room, safety office, inventory, documents, and shore operations into a premium maritime intelligence layer.</p><div class="stack-metrics">${metric('6','Active vessels','Six vessels are active across Atlantic, Gulf, and Pacific fleets.')} ${metric('3','Pending approvals','Captain approval is required on PTW, eORB, and purchase order records.')} ${metric('97%','Fleet health','Fleet health combines compliance, PMS, safety, certificates, and port-call readiness.')} ${metric('38h','Next ETA','Atlantic Pioneer arrives Santos in approximately 38 hours.')}</div></section><section class="map-card glass"><div class="card-title"><h2>Fleet Position</h2><button data-page="Navigation">Open Navigation</button></div><div class="fleet-map"><button class="map-pin" style="left:22%;top:38%" data-drawer="Atlantic Pioneer|ETA Santos +38h. PSC readiness review is critical and certificate pack is prepared.">Atlantic Pioneer</button><button class="map-pin" style="left:54%;top:25%" data-drawer="Pacific Meridian|ETA Singapore 4 days. Deck department has hot work permit awaiting Master signature.">Pacific Meridian</button><button class="map-pin" style="left:72%;top:56%" data-drawer="Aurora Spirit|At anchor. Engineering department has auxiliary generator work order waiting on parts.">Aurora Spirit</button><button class="map-pin" style="left:42%;top:70%" data-drawer="Northstar|Procurement queue has critical spares reorder and vendor quote pending.">Northstar</button></div></section><section class="split-grid"><div class="glass panel-block"><h2>Inventory Risk</h2>${barMetric('Below minimum stock','2','Two critical spares are below minimum stock. Create reorder or assign buyer.')} ${barMetric('Critical spares pending','19','Nineteen critical spare line items are waiting on vendor response or approval.')}</div><div class="glass panel-block"><h2>Critical Queue</h2>${queueRows()}</div></section><section class="glass panel-block"><div class="card-title"><h2>Recent Activity</h2><button data-popup="refresh">Refresh</button></div>${activity.map(a=>`<button class="activity" data-drawer="${safe(a[1])}|${safe(a[2])}"><b>${a[0]}</b><span>${a[1]}<small>${a[2]}</small></span></button>`).join('')}</section>`;
}

function pageTemplate(copy){
  return `<section class="page-head glass premium-glow"><p class="eyebrow">${page}</p><h1>${copy[0]}</h1><p>${copy[1]}</p><div class="action-row"><button class="primary small" data-drawer="${safe(copy[0])}|Open operational action center for ${safe(page)}. Assign an owner, attach evidence, set due dates, and record approvals.">Take Action</button><button data-popup="export">Export</button><button data-popup="comment">Comment</button></div></section>${page === 'Cargo' ? cargoPanel() : ''}${page === 'Port Calls' ? portPanel() : ''}${page === 'Fleet Overview' ? fleetPanel() : ''}${page === 'Vessel Profile' ? vesselProfilePanel() : ''}<section class="grid-cards">${moduleCards().map(c=>`<article class="glass module-card" data-drawer="${safe(c[0])}|${safe(c[2])}"><div class="module-icon">${c[3]}</div><h2>${c[0]}</h2><p>${c[1]}</p><span class="status">${c[4]}</span></article>`).join('')}</section><section class="glass panel-block"><h2>Audit Trail</h2>${activity.map(a=>`<button class="activity" data-drawer="${safe(a[1])}|${safe(a[2])}"><b>${a[0]}</b><span>${a[1]}<small>${a[2]}</small></span></button>`).join('')}</section>`;
}
...
function moduleCards(){ return [['Signed PTW Register','Safety critical records and authorization history.','Permit package, validation, audit trail, and comments are connected. Master and Chief Officer signoff states are visible.','▧','Current'],['Vessel Certificates','ISM, IOPP, MLC, safe manning and audit packs.','Certificate status, renewal dates, owners, alerts, and attached evidence are connected to the vessel record.','▰','Attention'],['Port Package','MOC crew list, stores declaration, dangerous goods forms.','Agent-ready port package generated from vessel records, cargo data, crew data, and document vault.','⌁','Pending'],['Maintenance Reports','PMS completion logs and machinery history.','Work order timeline with closeout evidence, running-hour triggers, defects, and parts reservations.','⚙','Current'],['Crew Readiness','Certificates, rest hours, medicals, and watch cover.','Crew compliance and readiness board with STCW, medical, watch, and travel exceptions.','♙','Good'],['Agent Links','All connected for port coordination.','Agent messages, local requirements, port-call records, attachments, and pending requests are centralized.','⛓','100%']]; }
...
function drawerHtml(){ if(!drawer) return ''; const fields = drawer.fields || [['Module',page],['Owner','Captain'],['Record','PTW #0047'],['Priority','Critical']]; const fieldHtml = fields.map(([label,value])=>`<label>${label}<input value="${safe(value)}" /></label>`).join(''); const quickHtml = drawer.quick ? `<div class="quick-grid">${drawer.quick.map(item=>`<button data-drawer="${safe(item)}|${safe(item)} workflow opened. In production this creates a record, assigns an owner, and starts an audit trail.">${item}</button>`).join('')}</div>` : ''; return `<div class="drawer-backdrop" data-close="true"></div><aside class="drawer glass"><button class="icon-btn close" data-close="true">×</button><p class="eyebrow">Operational Detail</p><h1>${drawer.title}</h1><p>${drawer.body}</p>${quickHtml}<div class="form-grid">${fieldHtml}</div><div class="action-row"><button class="primary small" data-toast="Action saved">Save Action</button><button data-popup="export">Export</button><button data-popup="comment">Comment</button><button data-toast="Evidence attachment queued">Attach Evidence</button></div><section class="audit"><h2>Audit Trail</h2>${activity.slice(0,2).map(a=>`<button class="activity" data-drawer="${safe(a[1])}|${safe(a[2])}"><b>${a[0]}</b><span>${a[1]}<small>${a[2]}</small></span></button>`).join('')}</section></aside>`; }
function render(){ document.getElementById('app').innerHTML = authed ? shell() : loginScreen(); }

document.addEventListener('click', e=>{ const target = e.target.closest('button,[data-page],[data-popup],[data-drawer],.side-link,.module-card,.scrim,.drawer-backdrop,.activity,.map-pin,.bar-metric,tr'); if(!target) return; if(target.dataset.page) return setPage(target.dataset.page); if(target.dataset.popup) return openPopup(target.dataset.popup); if(target.dataset.menu === 'open'){ sidebar = true; return render(); } if(target.dataset.menu === 'close'){ sidebar = false; return render(); } if(target.dataset.close) return closeDrawer(); if(target.dataset.search){ searchOpen = !searchOpen; drawer = null; sidebar = false; return render(); } if(target.dataset.toast) return toast(target.dataset.toast); if(target.dataset.drawer){ const split = target.dataset.drawer.split('|'); return openDrawer(split[0], split.slice(1).join('|') || 'Operational detail opened.'); } });
document.addEventListener('submit', e=>{ if(e.target.id === 'login-form'){ e.preventDefault(); authed = true; localStorage.setItem('neptune_auth','yes'); render(); } });
